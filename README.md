...
5. 「長期APIキーを生成」を押下
6. 生成したトークンを`.env`の`AWS_BEARER_TOKEN_BEDROCK`に設定

**認証情報を設定する場合**:
1. AWS IAMユーザーまたはロールの認証情報を用意
2. Bedrock Runtimeを呼び出せる権限が付与されていることを確認
...
# 特定のプラットフォームのみ
RUN_E2E_TESTS=true npm test -- --testPathPatterns=github.e2e
```
