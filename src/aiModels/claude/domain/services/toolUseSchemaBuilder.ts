import type { McpRegistry, PlatformName } from "@core/mcpRegistry";
import { mcpRegistry as getMcpRegistry } from "@core/mcpRegistry";
import type { ClaudeToolUseSchema } from "../repositories/claudeRepositoryRequestPayload";

export interface ToolUseSchemaBuilder {
  buildFromMcpNames: (mcpNames: PlatformName[]) => ClaudeToolUseSchema[];
}

/**
 * ToolUseスキーマビルダーを作成
 */
export const makeToolUseSchemaBuilder = (
  deps: {
  ...
  return {
    buildFromMcpNames: (mcpNames: PlatformName[]): ClaudeToolUseSchema[] => {
      return mcpNames.flatMap((mcpName) => {
