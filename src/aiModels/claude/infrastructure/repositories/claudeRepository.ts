...
import { ClaudeTextPayload } from "../../domain/repositories/claudeExtendedPayload";
import { ClaudeRepository } from "../../domain/repositories/claudeRepository";
import type {
  ClaudeContentBlock,
  ClaudeRequest,
  ClaudeSystemContentBlock,
  ClaudeToolUseSchema,
...
export const makeClaudeRepository = (
  ...
  return {
    ask: async (messages, tools, options) => {
      return safeCall<AiTextPayload | ClaudeTextPayload>(async () => {
        ...
        // Prompt Caching: system promptをarray形式にしてcache_controlを付与
        const enableCaching = options?.enablePromptCaching ?? false;
        let systemPayload: string | ClaudeSystemContentBlock[] | undefined;
        if (options?.system) {
          if (enableCaching) {
            systemPayload = [
              {
            ...
            systemPayload = options.system;
          }
        }

        // Prompt Caching: toolsの最後にcache_controlを付与
        let toolsPayload: ClaudeToolUseSchema[] | undefined;
        if (tools && tools.length > 0) {
          if (enableCaching) {
            toolsPayload = tools.map((tool, index) =>
        ...
        if (tools && tools.length > 0) {
          ...
          const usage = {
            ...
            ...(responseBody.usage.cache_creation_input_tokens != null && {
              cacheCreationInputTokens:
                responseBody.usage.cache_creation_input_tokens,
            }),
            ...(responseBody.usage.cache_read_input_tokens != null && {
              cacheReadInputTokens: responseBody.usage.cache_read_input_tokens,
            }),
          };

          const response: ClaudeTextPayload = {
            text: textContent,
            finishReason,
            usage,
        ...
        // usageはログに出すための情報なのでロジックでは使用されていない
        const usage = {
          promptTokens: responseBody.usage.input_tokens,
          completionTokens: responseBody.usage.output_tokens,
          totalTokens:
