...
import { executeGoogleDocsGetDocument } from "../../platforms/google/presentation/docsCliFormatter";
    ...
    mcpCommands.forEach((cmd) => {
      ...
      commands.push({
        ...
        execute: async (args: string[]) => {
          ...
          try {
            // Google DocsのgetDocumentの場合は専用の処理を実行
            if (
              platformName === "google" &&
              commandName === "getDocument" &&
              !isJsonOutput
            ) {
              await executeGoogleDocsGetDocument(args, call);
              return;
            }
...
export const showHelp = (): void => {
  ...
  console.log("");
  console.log("例:");
  console.log("  npm run cli help redmine");
  console.log("  npm run cli help github");
  console.log(
    '  npm run cli github:searchCode "functionName in:file language:ts"',
  );
  console.log("");
  console.log("初期設定:");
  console.log(
