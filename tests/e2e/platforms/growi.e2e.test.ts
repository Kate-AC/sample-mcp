...
describe("Growi Platform E2E Tests", () => {
  const platformName: PlatformName = "growi";
    ...
    commands.forEach((cmd) => {
      describe(`${cmd.command}`, () => {
        it(`${cmd.command} - isSuccessがtrueであること`, async () => {
          ...
          // createPageコマンドは実際にGrowiにページが作成されるためスキップ
          if (cmd.command.includes("createPage")) {
            console.log(
              `⏭️  ${platformName}:${cmd.command} - 実際にGrowiにページが作成されるため、E2Eテストをスキップします`,
            );
            return;
          }
