...
describe("Slack Platform E2E Tests", () => {
    ...
    commands.forEach((cmd) => {
      describe(`${cmd.command}`, () => {
        it(`${cmd.command} - isSuccessがtrueであること`, async () => {
          // postMessageコマンドは実際にメッセージが投稿されるためスキップ
          if (cmd.command.includes("postMessage")) {
            console.log(
              `⚠️  ${platformName}:${cmd.command} - 実際にメッセージが投稿されるためE2Eテストをスキップ`,
            );
            return;
          }
