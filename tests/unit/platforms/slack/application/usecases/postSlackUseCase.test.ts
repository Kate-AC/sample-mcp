...
describe("postSlackUseCase", () => {
  ...
  const mockSlackEnv: SlackEnv = {
    userOAuthToken: "dummy-slack-user-token",
    botUserOAuthToken: "dummy-slack-bot-token",
  ...
  describe("invoke", () => {
    ...
    it("デフォルトのファクトリを使用した場合に正常に動作すること", async () => {
      ...
      jest
        .spyOn(
          require("@platforms/slack/infrastructure/env/slackEnv"),
          "loadSlackEnv",
        )
        .mockReturnValue(mockSlackEnv);
