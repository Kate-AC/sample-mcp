...
describe("slackRepository", () => {
  ...
  describe("getThreadMessages", () => {
    ...
    it("フルURLでもスレッドメッセージを取得できること", async () => {
      ...
      const repository = makeSlackRepository();
      const result = await repository.getThreadMessages(
        "https://example.slack.com/archives/C09L24UTM8A/p1760259631615889",
      );
  ...
  describe("addReaction", () => {
    it("メッセージにリアクションを追加できること", async () => {
      ...
      const result = await repository.addReaction(
        "https://workspace.slack.com/archives/C09L24UTM8A/p1760259631615889",
        "white_check_mark",
      );

      expect(mockApiClient.post).toHaveBeenCalledWith("/reactions.add", {
        channel: "C09L24UTM8A",
        timestamp: "1760259631.615889",
        name: "white_check_mark",
      });
    ...
    it("パス形式のURLでもリアクションを追加できること", async () => {
      const mockResponse = {
        ok: true,
      };
      ...
      const repository = makeSlackRepository();
      const result = await repository.addReaction(
        "/archives/C1234567890/p1234567890123456",
        "thumbsup",
      );
    ...
    it("無効なURL形式の場合はエラーを返すこと", async () => {
      const repository = makeSlackRepository();
      const result = await repository.addReaction("invalid-url", "thumbsup");

      expect(result.isSuccess).toBe(false);
      expect(result.message).toContain("Invalid Slack message URL format");
    });

    it("チャンネルIDが含まれていないURLの場合はエラーを返すこと", async () => {
      const repository = makeSlackRepository();
      const result = await repository.addReaction(
        "/archives/p1234567890123456",
        "thumbsup",
      );
    ...
    it("タイムスタンプが含まれていないURLの場合はエラーを返すこと", async () => {
      const repository = makeSlackRepository();
      const result = await repository.addReaction(
        "/archives/C1234567890",
        "thumbsup",
      );
